...
class AuthControllerTest {
    ...
    @Test
    @DisplayName("토큰_갱신_API_문서화_테스트")
    void 토큰_갱신_API_문서화_테스트() throws Exception {
        // given
        TokenResponse tokenResponse = new TokenResponse("new-access-token-example");
        ResultDTO<TokenResponse> resultDTO = ResultDTO.success(tokenResponse, "token-refresh-success");

        when(jwtTokenProvider.getRefreshTokenFromCookie(any())).thenReturn("valid-refresh-token");
        when(authService.refresh(anyString())).thenReturn(resultDTO);
